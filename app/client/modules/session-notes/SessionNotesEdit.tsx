...
export function SessionNotesEdit() {
  const { fileId } = useParams<{ fileId: string }>();
  const navigate = useNavigate();
  const { get, create, update } = useFiles('session-notes');
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (isNew) {
        ...
        navigate(`/modules/session-notes/${newNotes.frontmatter.id}`);
      } else {
        ...
        navigate(`/modules/session-notes/${fileId}`);
      }
    } catch (error) {
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6">
      {/* Back link */}
      <Link
        to={isNew ? '/modules/session-notes' : `/modules/session-notes/${fileId}`}
      ...
      <div className="space-y-4 rounded-lg border border-border bg-card p-6">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <div>
            ...
            <input
              ...
              placeholder="e.g., DM or Player Name"
              className="w-full rounded-md border border-input bg-background px-3 py-2 text-foreground placeholder:text-muted-foreground focus:border-ring focus:outline-none focus:ring-1 focus:ring-ring"
            />
          </div>
        </div>
        ...
        <div>
          ...
          <textarea
            ...
            rows={20}
            placeholder="Write your session notes here. You can use Markdown formatting and link to other entries with [[module:id]] syntax."
