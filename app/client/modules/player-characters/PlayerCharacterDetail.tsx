...
import {
  ...
  ExperienceTracker,
  LuckTracker,
} from './components/TrackerRow';
import { GearList } from './components/GearList';
import { PlaybookMoveGrid } from './components/PlaybookMoveCard';
import type { PlayerCharacterFrontmatter, Stats, ResourceLevel, HarmState, GearItem } from '@shared/schemas/player-character';
  ...
  const { data: moves } = useQuery({
    ...
    queryFn: async () => {
      ...
      const res = await fetch(
        `/api/modules/player-characters/${fileId}/moves`
      );
      ...
      <div className="flex items-start justify-between">
        <div className="flex gap-4">
          ...
          <div>
            ...
            <div className="mt-2">
              <CopyableId moduleType="player-characters" id={fileId || ''} />
            </div>
          </div>
        </div>
        ...
        <div className="flex gap-2">
          <Link
            to={`/modules/player-characters/${fileId}/edit`}
