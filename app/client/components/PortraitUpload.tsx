...
export function PortraitUpload({
  ...
  const [position, setPosition] = useState<Position>(
    portraitPosition || { x: 0, y: 0, scale: 1 }
  );
  ...
  // Build the portrait URL
  const getPortraitUrl = useCallback(() => {
    if (previewUrl) return previewUrl;
    if (currentPortrait && campaign) {
      return `/api/campaigns/${campaign.id}/assets/${currentPortrait.replace('assets/', '')}`;
    }
    return null;
  }, [previewUrl, currentPortrait, campaign]);
  ...
  const handleDragStart = (e: React.MouseEvent) => {
    if (!previewUrl && !currentPortrait) return;
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/campaigns/${campaign.id}/portraits/${npcId}`,
        {
          method: 'POST',
          body: formData,
        }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-start gap-4">
        <div
          ref={previewRef}
          className="relative h-32 w-32 shrink-0 overflow-hidden rounded-full border-2 border-border bg-secondary"
          style={{ cursor: portraitUrl ? 'move' : 'default' }}
        ...
        <div className="flex-1 space-y-3">
          ...
          <div>
            ...
            >
              ...
              {currentPortrait || selectedFile ? 'Change Image' : 'Upload Image'}
            </label>
          </div>
